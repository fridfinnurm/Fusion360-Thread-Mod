...
threadsizes = list(map(lambda e: float(e.text), threadlib.getroot().findall(".//ThreadSize/Size")))
print(threadsizes)
...
for ts in threads:
    ...
    for designation in ts.iterfind(".//Designation"):
        ...
        for thread in designation.iterfind(".//Thread"):
            ...
            majordia = round(float(thread.find(".//MajorDia").text) + adjustment, 3)
            pitchdia = round(float(thread.find(".//PitchDia").text) + adjustment, 3)
            minordia = round(float(thread.find(".//MinorDia").text) + adjustment, 3)
